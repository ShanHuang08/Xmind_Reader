"""Create API-based section chunks from PDF Markdown."""
...
def chunk_sections(markdown: str, endpoint_index: list[dict[str, Any]], source_pdf: str) -> list[dict[str, Any]]:
    ...
    for index, endpoint in enumerate(ordered):
        ...
        if not content:
            content = _fallback_window(lines, start)
...
def _fallback_window(lines: list[str], start: int) -> str:
    return "\n".join(lines[max(0, start - 8) : min(len(lines), start + 30)]).strip()
